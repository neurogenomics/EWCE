...
This repository for *EWCE* is now deprecated. Please install EWCE from Bioconductor (3.13, R 4.1). See [NathanSkene/EWCE](https://github.com/NathanSkene/EWCE) for the new repository and further information. One thing to note is that all functions which previously used dot '.' notations to separate words have been updated to underscores '_' (e.g, `bootstrap.enrichment.test()` is now `bootstrap_enrichment_test()`).  The package [vignette website](https://nathanskene.github.io/EWCE/articles/EWCE.html) has been updated to this new version of EWCE and gives further details.
...
The *EWCE* package is available from github. There is a version of EWCE available in the Bioconductor archives but it is depreciated and should not be used. A separate repository of EWCE is currently being added to Bioconductor and should be available there from May 2021. In the interim, use the neurogenomics/EWCE repository. To be able to install the package one needs first to install R then run the following lines of code:
